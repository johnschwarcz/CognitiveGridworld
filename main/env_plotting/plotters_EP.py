...
class Plotters(Plotting_anim):
    ...
    def plot_model_perf(self):
        ...
        ax[2].plot(self.test_accs[1:self.test_e, -1], c = 'r', label = "model test")
        ax[2].axhline(self.agent_accs[0].mean(0)[-1], ls = '--', c = 'C1', label = "joint")
        ax[2].axhline(self.agent_accs[1].mean(0)[-1], ls = '--',c = 'C2', label = "naive")
        ax[2].legend()
        plt.show()      
...
if __name__ == "__main__":
    ...
    base_args = {'mode': "RL", 'cuda': cuda, 'load_env': 'RL', 'show_plots': False,
    ...
    joint = CognitiveGridworld(**base_args)
    naive = CognitiveGridworld(**base_args)
    online_net = CognitiveGridworld(**base_args)
    offline_net = CognitiveGridworld(**base_args)

    # New control: online agent with a fresh network per episode that also trains generator/embeddings each episode
    fresh_args = base_args.copy()
    fresh_args.update({'reinit_each_episode': True, 'train_generator_each_episode': True, 'training': True})
    online_fresh = CognitiveGridworld(**fresh_args)
    ...
    # Train the new fresh-online control
    online_fresh.train_controller(eps = eps, reps = reps)
    if save:
        with open(os.path.join("main//DATA//controller", "online_fresh.pkl"), 'wb') as f:
            pickle.dump(online_fresh.controller_training_logs, f)
